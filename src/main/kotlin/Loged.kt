...
fun Loged.f(
    ...
    "${if (UNIT_TESTING) "" else "$infoText\n"}${msg.toString().frame(FrameType.BOX.copy(top = tag, bottomLeft = "╠"))}",
    tag,
    showPretty,
    threadName
)
...
fun Loged.f(
    ...
    "${if (UNIT_TESTING) "" else "$infoText\n"}${msg.frame(FrameType.BOX.copy(top = tag, bottomLeft = "╠"))}",
    tag,
    showPretty,
    threadName
)
...
fun <T> Loged.f(
    ...
    "${if (UNIT_TESTING) "" else "$infoText\n"}${msg.frame(FrameType.BOX.copy(top = tag, bottomLeft = "╠"), transform = transform)}",
    tag,
    showPretty,
    threadName
)
